...
import { useLazyGetProjectsQuery } from "@/features/search/api/searchApi";
import FilterPagination from "@/features/search/components/FilterPagination";
import { useEffect, useState } from "react";

import ProjectResults from "@/features/search/components/ProjectResults";
...
type SearchType = "projects" | "categories" | "users";
...
const ProjectSearch = () => {
  ...
  const [fetchMovies, { data: movies, isLoading: projectsLoading }] =
  ...
  useEffect(() => {
    ...
    switch (selectedType) {
      case "projects":
        fetchMovies({ title: searchName.toLowerCase() });
        break;
  ...
  }, [searchName, selectedType, categoriesData]);

  const getCurrentData = () => {
    if (selectedType === "projects") return movies || [];
    ...
    <div className="min-h-screen bg-[#8F92A10D] rounded-tl-3xl py-10 overflow-hidden">
      ...
      <div className="bg-white pt-4">
        <FilterPagination
          ...
          onChangeType={(type: SearchType) => setSelectedType(type)}
        />
      </div>

      {/* Контент в зависимости от фильтра */}
      {isLoading ? (
        <div className="p-12">Загрузка...</div>
      ) : (
        <>
          {selectedType === "projects" && (
            <ProjectResults data={movies ?? null} />
          )}
          {selectedType === "categories" && (
            <CategoryResults data={filteredResults ?? []} />
          )}
          {selectedType === "users" && (
            <UserResults data={filteredResults ?? []} />
          )}
        </>
      )}
    </div>
